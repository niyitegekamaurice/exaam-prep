...
<form action="periodicreportout.php" method="POST">
    <fieldset>
        <h2>enter date </h2>
...
if (isset($_POST['check'])) {
    ...
    SELECT 
        product.productCode, 
        product.productName, 
        DATE(productout.Dateout) AS Date,
        SUM(productout.Quantity_out) AS totalQuantity, 
        SUM(productout.Totalprice_out) AS totalPrice
    FROM 
        product
    JOIN 
        productout ON product.productCode = productout.productCode
    WHERE 
        productout.Dateout BETWEEN '$fromdate' AND '$todate'
    GROUP BY 
        product.productCode, product.productName, DATE(productout.Dateout)
    ";

    $result = mysqli_query($conn, $sql);

    if ($result && mysqli_num_rows($result) > 0) {
        echo "<h2 style='text-align: center;font_size:20px;'>Report  of productout from { $fromdate} to {$todate}</h2>";
        ?>
        <table border='3'>
            <tr>
                <th>Product Code</th>
